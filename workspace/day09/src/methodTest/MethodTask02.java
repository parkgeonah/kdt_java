   ...
   boolean contais(int[] ar) {
	 ...
	 //6. 두개의 문자열을 입력받아서 두 문자열의 길이가 같으면 같음, 다르면 다름 반환하는 메소드
	   ...
	   //7. 아이디와 비밀번호를 매개변수로 받아 검증, 시도횟수 3회 넘기면 잠금 반환
		    //loginTry
	   // 로직구성
	   // 1. 기능생각 => 아이디 비밀번호를 받아 로그인시도 3회넘기면 잠금
	   // 2. 메소드명  loginTry
	   // 3. 문자열타입 매개변수 2개 만들기 
	   // 4. id, pw 문자열 변수 만들기, 로그인 시도횟수 정수 변수 만들기
	   // 5. while문을 통해 3번 반복할수 있게 만들기 (로그인 시도횟수 < 3)
	   // 6. if문을 통해 아이디 비밀번호 검증
	   // 7. else문을통해 틀리면 다시 시도 3번틀리면 잠금반환
	   // 8. 리턴타입 반환.
